...
const starting_menu = {
    ...
    grantAccess: command(/(G|g)rant access \w+/, (msg) => {database.validateUser(msg.text.split(' ')[2]); msg.reply.text('Please ask the new user to use the command \'Validate\' on me.')}) // grant access [telegram_username]
}
...
async function validate(msg){
    if((await database.userIsValidated(msg.chat.username))){
